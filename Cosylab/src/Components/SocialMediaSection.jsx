...
const SocialMediaSection = () => {
  ...
  useEffect(() => {
    fetch(`${VITE_API_BASE_URL}/social-media`)
      .then((res) => {
        if (!res.ok) throw new Error(`Fetch error ${res.status}`);
        return res.json();
      })
      .then((data) => {
        const tw = data.filter((p) => p.platform === "twitter").map((p) => p.url);
        const li = data.filter((p) => p.platform === "linkedin").map((p) => p.url);
        setTweets(tw);
        setLinkedin(li);
      })
  ...
  // Re-run twitter widgets load on tweets update to render embedded tweets properly
  useEffect(() => {
